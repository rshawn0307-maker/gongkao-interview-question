...
知识库根目录按以下优先级确定：
1. 用户明确指定了路径 → 直接使用该路径
2. 当前工作目录下存在 `01_题库与训练/` → 使用该子目录
3. 以上都不存在 → 在当前工作目录下创建 `01_题库与训练/<题型>题/` 后写入
...
🔴 **CHECKPOINT · 🛑 STOP：必须等用户明确确认"风格 OK / 可以批量"后，才进入步骤4。用户未确认前禁止批量生产。**

4. 用户确认后批量生产剩余题目
5. 批量生产时每批用脚本批量验证：`python <skill目录>/scripts/validate_question.py 综合分析_*.md`
6. 每批验证不通过的文件单独修正后重跑，不要混入下一批
